...
- ~~`ALTER TABLE <t> ADD [CONSTRAINT <name>] CHECK (<expr>)` con re-validación O(n) de todas las filas existentes antes de persistir. Sin estado parcial: cualquier fila que viole el predicado nuevo aborta el ALTER con `[GBY-3008]` y la PK ofensiva. Sin bump de formato (el slot ya estaba en V10)~~ ✅ entregado (sub-bloque **L3** / residual #1 de L, 2026-05-27). Sub-pendientes restantes del bloque L: nombres en PK/UNIQUE/FK (residual #2), multi-col FOREIGN KEY (residual #3), activación real de `ON UPDATE` (residual #4) — todos diferidos.
...
> Para el **inventario exhaustivo de comandos SQL que faltan** (con prioridades P0–P3 y la secuencia de bloques cerrados `E1 → E2 → E3 → F → T → J → J2 → G1 → G2 → G3 → H → I → K1 → K2 → L1 → L2 → L3` y pendientes `V → W → X → Y → Z`), ver [docs/MISSING_COMMANDS.md](docs/MISSING_COMMANDS.md).
