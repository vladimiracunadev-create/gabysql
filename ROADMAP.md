...
> Para el **inventario exhaustivo de comandos SQL que faltan** (con prioridades P0–P3 y la secuencia de bloques cerrados `E1 → E2 → E3 → F → T → J → J2 → G1 → G2 → G3 → H → I → K1 → K2 → L1` y pendientes `L2 → V → W → X → Y → Z`), ver [docs/MISSING_COMMANDS.md](docs/MISSING_COMMANDS.md).
