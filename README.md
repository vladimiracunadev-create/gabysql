...
| 🧭 | [COMO_TRABAJAMOS](docs/COMO_TRABAJAMOS.md) | **explica las letras** (P5b, R6, M2…) y cómo nace/se entrega cada bloque. Léelo si te perdés con tanta nomenclatura |
...
> **Estado**: 🟢 Fase 1 cerrada · Fase 2 cerrada (SQL relacional completo + constraints + vistas + CTEs + window functions + triggers + procedures + user functions + PL/pgSQL completo + tipos extendidos con DECIMAL/BLOB/UUID exactos + USERS/ROLES + GRANT/REVOKE + Row-Level Security) · **Fase 3 cerrada (2026-06-11)** con `P1`+`P2`+`P3`+`P3b`+`P4`+`P5a`+`P5b`+`P5c`+`P5d`+`P5e` + post-Fase 3 hardening (`R1`+`R4`+`R6`+`R8`+`M2`) + **sesión maratón 2026-06-15** (R7+R9+R10+R2+R3+R3-cont+ANSI fix+M3+M4+M6+M12+M13 + Pager proptest). VERSION on-disk 33. **Marcadores actuales: 828/828 tests verdes** (818 integration + 4 server E2E + 3 planner proptest + 3 pager proptest + 4 ignored: 1 Argon2id RFC + 2 env-var serial + 1 fuzz parser on-demand) · CI verde en Ubuntu/macOS/Windows + Docker + bench job · gabybench all 12.9 min, 71 queries, 0 SKIPs · **M4 fuzz parser** = 503.8M queries random, 1h limpia, 0 panics ([evidencia](docs/fuzz/FUZZ-RUN-2026-06-15.md)).  
...
> **Superficie SQL** (TCL): `BEGIN` / `START TRANSACTION` / `COMMIT` / `END` / `ROLLBACK` batch-local + `SAVEPOINT name` / `ROLLBACK TO [SAVEPOINT] name` / `RELEASE [SAVEPOINT] name` (M12, 2026-06-15, ADR-0089) + cross-request HTTP sessions via `X-Gabysql-Session` header (M13, 2026-06-15, ADR-0090)  
...
> **Superficie SQL** (P, Fase 3 completa, 2026-05-29 → 2026-06-11 — planeación): `EXPLAIN <stmt>` describe el plan como ResultSet textual sin ejecutar (P1), `EXPLAIN ANALYZE <stmt>` ejecuta el inner real + `std::time::Instant` wall-clock + row count real + error captura (P2; **side-effects PERSISTEN** — usar EXPLAIN sin ANALYZE para dry-run), `ANALYZE [TABLE] <name>` colecta `row_count` + `analyzed_at_nanos` + per-column stats (`null_count` exacto + NDV vía HyperLogLog 256-reg con splitmix64 finalizer + MCV top-K=10 + histograma equi-depth ~16 buckets) y **los persiste en el catálogo** vía `ObjectKind::TableStats` (P3b+P4) — sobreviven a reopen del Engine, se borran con DROP TABLE; EXPLAIN consume las stats como `[est.rows=N cols=M est.match=K stats.age=Xd Yh]` (P3+P3b+P4+P5a+R1), `EXPLAIN` anota el algoritmo real de JOIN (index-loop / hash join / nested-loop, P5e) y composite index lookup (P5b). El planner usa las stats para: cost-based skip-index si selectividad ≥ 20% (P5c, con bypass automático si stats stale > 7 días — R1), composite secondary index lookup en SELECT/UPDATE/DELETE (P5b+R8), post-lookup bucket-size check para refinar la asunción de independencia AND (R6), hash-join build-side selection por cardinalidad real (P5d).

> **Persistencia**: `.db` + `.wal` con recovery por `COMMIT`, checksums CRC32 por página, crash tests dirigidos  
> **Formato en disco**: `VERSION = 33` — último bump: **33 = column stats (P4, 2026-06-10)** — extiende el record `ObjectKind::TableStats` con `format_version (u8) + column_count (u16) + Vec<ColumnStats>` (ver [ADR-0068](docs/adr/0068-p4-column-stats.md)). Bump previo: **32 = `ObjectKind::TableStats` (P3b, 2026-06-09)** — persiste stats por tabla bajo clave `__stats__:<tabla>` (ver [ADR-0067](docs/adr/0067-p3b-persistent-stats.md)). Bumps previos: 23 = `ObjectKind::User/Role` (Z1), 24 = `Grant` (Z2), 25 = `Policy` USING (Z3), 26 = UserMeta PBKDF2 (Z1b), 27 = PolicyMeta con `with_check_sql` + `FOR INSERT` (Z3b), 28 = scrypt scheme=2 (Z1c), 29 = Blake2b H' (Z1d), 30 = Argon2id estructural scheme=3 (Z1e), 31 = Argon2id partial fix (Z1f). Bumps previos vigentes: Y6 (DECIMAL exacto, i128+scale), Y5 (UNSIGNED en int_width high bit), Y4 (BLOB), Y3 (int_width TINY/SMALL/MEDIUM/INT4), Y2 (max_length VARCHAR/CHAR), Y (TIME+UUID), X3b (`Function`), X3 (`Procedure`), V (discriminator tabla/vista), L1 (`ON UPDATE` + SET NULL/DEFAULT), L2 (CHECK trailer), K2 (PK + índices compuestos via fingerprint FNV-1a-64). B+Tree real, catálogo FNV-1a-64, índices secundarios `unique` + `IndexKind` Hash/OrderedInt. **Todos los bloques P4 → R6 (10 bloques) son zero-bump excepto P4 — la lógica del optimizer no toca el layout on-disk.**  
> **Portabilidad**: Windows, Linux y macOS por CI · **850 tests verdes** (818 integration + 13 catalog listing E2E + 9 log del motor E2E + 4 server E2E M13 + 3 proptest planner M3 + 3 proptest Pager) — 3 redes property-based defienden correctness en planner cost-based, storage layer y server HTTP, todas con seed reproducible · **M4** fuzz parser 503.8M queries random / 1h limpia / 0 panics · `/metrics` + `-log-json` para observabilidad del transporte y **log de sentencias del motor** (`-log-file`, estilo `log_statement` de PostgreSQL, con rotación) para el SQL y sus errores · `gabysql backup/restore/verify` con CRC end-to-end · job CI `bench` corre `gabybench smoke` y sube `bench/results.json` como artifact por commit (M2) · **TCL completo**: `BEGIN`/`COMMIT`/`ROLLBACK` + `SAVEPOINT`/`ROLLBACK TO`/`RELEASE` (M12 ANSI SQL:2003) · **Sessions cross-request HTTP** (M13): ORMs pueden hacer `BEGIN`/`INSERT`/`INSERT`/`COMMIT` a través de N requests separados con `X-Gabysql-Session` header · semántica ANSI alineada: `UPDATE/DELETE WHERE pk no-existe` devuelve `0 filas` (no error) · **M6** `EXPLAIN ANALYZE` anota bias del estimator (`est.match` vs `actual` con clasificación GOOD/MILD/HIGH)  
> **Performance** (smoke bench 2026-06-11 en CI Linux, microblog 50k filas + orders_lines 120k filas — corrida completa en [BENCHMARK.md](BENCHMARK.md)): PK lookup hot **12 µs**, PK compuesta full match **11 µs**, composite index lookup `qty+precio` (P5b cierra ADR-0066 Gap 10) **8.7 µs** vs 161 ms pre-P5b (**~18 500×**), index secundario eq **8 µs**, INSERT/UPDATE/DELETE por PK in-tx **15-19 µs**. Workloads sin índice: full scan TEXT LIKE A% **769 ms**, BETWEEN sin idx (100k) **117 ms**, aggregate SUM(qty*precio) GROUP BY **0.25 ms**. **10/10 gaps del bench cerrados** (ver [ADR-0066](docs/adr/0066-bench-exposed-gaps.md)).  
