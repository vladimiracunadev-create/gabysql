...
> **Superficie SQL** (W/X/Y, 2026-05-29): `WITH [RECURSIVE] cte AS (...)` (W1+W2), window functions `ROW_NUMBER`/`RANK`/`DENSE_RANK`/`LAG`/`LEAD`/`FIRST_VALUE`/`LAST_VALUE`/`SUM|AVG|MIN|MAX|COUNT OVER` (W3), `CREATE TRIGGER [BEFORE|AFTER] {INSERT|UPDATE|DELETE} ON t FOR EACH ROW BEGIN ... END` (X1+X2), `CREATE PROCEDURE name(params) AS BEGIN ... END` + `CALL` (X3), `CREATE FUNCTION name(params) RETURNS type AS <expr|BEGIN..END>` invocable en SELECT/WHERE (X3b+X4f), control de flujo procedural completo `IF/CASE/WHILE/FOR/LOOP/DECLARE/SET/RAISE/EXCEPTION/RETURN` (X4 → X4f), tipos extendidos con aliases (`BIGINT`/`VARCHAR(n)`/`DECIMAL(p,s)`/`DOUBLE PRECISION`/`BOOLEAN`/`TIMESTAMP`/…) + nuevos `TIME` y `UUID` (Y), enforcement `VARCHAR(n)`/`CHAR(n)` en bytes UTF-8 (Y2)  
> **Persistencia**: `.db` + `.wal` con recovery por `COMMIT`, checksums CRC32 por página, crash tests dirigidos  
> **Formato en disco**: `VERSION = 22` (Y6: tipo `DECIMAL(p,s)`/`NUMERIC(p,s)` exacto con code=11 + `Value::Decimal { value: i128, scale: u8 }` + encoding propio i128 LE + scale por fila + per-column `(precision, scale)`; Y5: `int_width` con high bit 0x80 = UNSIGNED; Y4: tipo `BLOB`/`BYTEA`/`BINARY` con code=10 + `Value::Bytes` + encoding propio u32 LE length + literal `X'hex'`; Y3: `int_width` opcional por columna para enforcement TINYINT/SMALLINT/MEDIUMINT/INT4; Y2: `max_length` opcional por columna para enforcement `VARCHAR(n)`/`CHAR(n)`; Y: tipos `TIME` y `UUID` con código en disco; X3b: `ObjectKind::Function`; X3: `ObjectKind::Procedure`; V: discriminator byte tabla/vista en cada record del catálogo; L1: `ON UPDATE` byte + `OnDelete` extendido a SET NULL/SET DEFAULT; L2: trailer de CHECK constraints; residual #2: nombres PK/FK opcionales; residual #3: extras source/target multi-col en cada FK; K2: PK compuesta + índices compuestos via fingerprint FNV-1a-64; B+Tree real, hash de catálogo FNV-1a-64, índices secundarios + `unique` flag + `IndexKind` Hash/OrderedInt)  
> **Portabilidad**: Windows, Linux y macOS por CI · 605 integration tests verdes · `/metrics` + `-log-json` para observabilidad básica · `gabysql backup/restore/verify` con CRC end-to-end · `WHERE col_int_idx BETWEEN a AND b` con índice ordenado  
