...
> **Formato en disco**: `VERSION = 21` (Y5: `int_width` con high bit 0x80 = UNSIGNED para TINYINT/SMALLINT/MEDIUMINT/INT4/BIGINT; Y4: tipo `BLOB`/`BYTEA`/`BINARY` con code=10 + `Value::Bytes` + encoding propio u32 LE length + literal `X'hex'`; Y3: `int_width` opcional por columna para enforcement TINYINT/SMALLINT/MEDIUMINT/INT4; Y2: `max_length` opcional por columna para enforcement `VARCHAR(n)`/`CHAR(n)`; Y: tipos `TIME` y `UUID` con código en disco; X3b: `ObjectKind::Function`; X3: `ObjectKind::Procedure`; V: discriminator byte tabla/vista en cada record del catálogo; L1: `ON UPDATE` byte + `OnDelete` extendido a SET NULL/SET DEFAULT; L2: trailer de CHECK constraints; residual #2: nombres PK/FK opcionales; residual #3: extras source/target multi-col en cada FK; K2: PK compuesta + índices compuestos via fingerprint FNV-1a-64; B+Tree real, hash de catálogo FNV-1a-64, índices secundarios + `unique` flag + `IndexKind` Hash/OrderedInt)  
> **Portabilidad**: Windows, Linux y macOS por CI · 567 integration tests verdes · `/metrics` + `-log-json` para observabilidad básica · `gabysql backup/restore/verify` con CRC end-to-end · `WHERE col_int_idx BETWEEN a AND b` con índice ordenado  
