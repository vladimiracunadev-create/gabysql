# 📖 Manual de usuario · gabymodeler v3

> **Modelador ER → DDL** para `gabysql`. Single-page HTML+CSS+JS vanilla, sin frameworks ni servidor obligatorio. Espejo del motor `gabysql VERSION 33`.
>
...
> ```
> Desde `gabysql VERSION 33` el server ya emite headers CORS, así que el modeler en `:8000` puede hablarle al API en `:8080` sin proxy.
...
4. **Header informativo**: las dos primeras líneas son comentarios con el timestamp y el target (`gabysql VERSION 33`).
...
| :--- | :--- | :--- |
| **API base URL** | `http://localhost:8080` | El endpoint del `gabysql-server`. Desde gabysql VERSION 33 ya tiene CORS habilitado. |
