...
| Funciones escalares (`LENGTH`, `UPPER`, `LOWER`, `SUBSTR`, `CONCAT`, `ABS`, `ROUND`, `NOW`, `CURRENT_DATE`, `CURRENT_TIMESTAMP`, `COALESCE`, `NULLIF`, `IFNULL`, `IF`, `CAST`, `CASE`) | 🟡 | Bloque G1 (2026-05-26): solo en SELECT list, con NULL propagation y 3VL para `CASE searched`. **Limitación**: no se aceptan en `WHERE`/`HAVING`/`UPDATE SET` (bloque G2). Operador `\|\|` para concat no se soporta (usar `CONCAT`). | [src/sql.rs](../src/sql.rs) |
