...
| Funciones escalares (`LENGTH`, `UPPER`, `LOWER`, `SUBSTR`, `CONCAT`, `ABS`, `ROUND`, `NOW`, `CURRENT_DATE`, `CURRENT_TIMESTAMP`, `COALESCE`, `NULLIF`, `IFNULL`, `IF`, `CAST`, `CASE`) | 🟢 | Bloques G1+G2 (2026-05-26): disponibles en SELECT list **y** en `WHERE` / `HAVING` / `UPDATE SET` / `DELETE WHERE` / `ON CONFLICT DO UPDATE SET`. NULL propagation + 3VL en CASE searched y en el WHERE. **Limitaciones**: operadores postfix sobre expresión escalar (`LENGTH(x) IS NULL`, `UPPER(x) LIKE 'A%'`, `LENGTH(x) IN (...)`, `LENGTH(x) BETWEEN ... AND ...`) aún no se soportan — `[GBY-4039]` con guía. Aritméticos binarios (`+`/`-`/`*`/`/`), operador `\|\|`, y funciones P2/P3 (TRIM, REPLACE, CEIL/FLOOR, MOD, DATE_ADD/SUB, EXTRACT, ...) quedan para G3. `EXCLUDED.col` en UPSERT sigue sin soporte. | [src/sql.rs](../src/sql.rs) |
