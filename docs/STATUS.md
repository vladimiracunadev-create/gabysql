...
[![Tests integraci%C3%B3n](https://img.shields.io/badge/tests-850%2F850-brightgreen)](../tests/integration_test.rs)
...
| Observabilidad del server | 🟢 | Endpoint `/metrics` (uptime, counts por status, errors_total, p50/p95 latencia) + flag `-log-json` para logs JSONL por request. Describe el **transporte HTTP**, no el SQL. Ver [ADR-0014](adr/0014-logs-json-metrics.md). | [src/server.rs](../src/server.rs) |
| Log de sentencias del motor | 🟢 | Bloque L (2026-08-13, zero-bump). Equivalente a `log_statement`/`log_min_error_statement` de PostgreSQL. Sink JSONL append-only con rotación por tamaño (default 8 MiB × 3 archivos), enganchado en `Engine::exec` → cubre CLI, REPL, server, gateway MCP y uso embebido con un solo hook. Niveles `none`/`error` (default)/`mod`/`all`. Cada entrada lleva `kind`, `sql` completo, `ok`, código `[GBY-NNNN]` extraído, `rows`, `duration_us` y `v` de schema. Guard de anidamiento: un `CALL` con loop escribe **una** línea, no una por iteración. Config: `-log-file`/`-log-level` en el server, `GABYSQL_LOG_FILE`/`GABYSQL_LOG_LEVEL` en ambos. Ver [ADR-0094](adr/0094-engine-statement-log.md). | [src/dblog.rs](../src/dblog.rs), [src/sql.rs](../src/sql.rs) |
