...
| :--- | :--- | :--- |
| `32` | 🟢 Actual | Bloque P3b (2026-06-09). Nuevo `ObjectKind::TableStats` (code=9) que persiste `StatsMeta { name, row_count, analyzed_at_nanos }` bajo clave `__stats__:<tabla>`. ANALYZE upsert al catálogo; `Engine::new` hidrata el HashMap automáticamente; DROP TABLE borra el record. EXPLAIN sigue mostrando `[est.rows=N]`. Las stats sobreviven a reopen del Engine — cierra la limitación honesta de P3. Ver [ADR-0067](docs/adr/0067-p3b-persistent-stats.md). Rechaza VERSION 31 con `[GBY-1003]`. |
