...
| :--- | :--- | :--- |
| `33` | 🟢 Actual | Bloque P4 (2026-06-10). Extiende `StatsMeta` con sufijo aditivo `format_version (u8) || column_count (u16) || Vec<ColumnStats>` donde cada `ColumnStats` lleva `null_count`, `ndv` (estimado HyperLogLog 256-reg con splitmix64 finalizer), `mcv` (top-K=10 por frecuencia), `histogram` (equi-depth ~16 buckets sobre reservoir sample de hasta 10k filas). El `format_version` interno permite extensiones aditivas futuras sin bump global. ANALYZE produce stats por-columna; EXPLAIN muestra `[est.rows=N cols=M est.match=K stats.age=...]` (P5a+R1). Ver [ADR-0068](docs/adr/0068-p4-column-stats.md). Rechaza VERSION 32 con `[GBY-1003]`. |
| `32` | 🔴 Rechazado | Bloque P3b (2026-06-09). Nuevo `ObjectKind::TableStats` (code=9) que persiste `StatsMeta { name, row_count, analyzed_at_nanos }` bajo clave `__stats__:<tabla>`. ANALYZE upsert al catálogo; `Engine::new` hidrata el HashMap automáticamente; DROP TABLE borra el record. EXPLAIN mostraba `[est.rows=N]`. Las stats sobreviven a reopen del Engine — cerró la limitación honesta de P3. Ver [ADR-0067](docs/adr/0067-p3b-persistent-stats.md). Reemplazado por VERSION 33 (P4). |
| `31` | 🔴 Rechazado | Bloque Z1f (2026-05-29). Corte semántico tras el `same_slice_extra` partial fix de Argon2id. Output del Argon2id cambia significativamente pero **aún no matchea RFC 9106 §A.3** — Z1g pendiente. Default sigue siendo scrypt (Z1c, scheme=2). VERSION 31 puro corte de incompatibilidad — sin nuevo on-disk field (ADR-0061). Migrar via export/import. |
| `30` | 🔴 Rechazado | Bloque Z1e (2026-05-29). Argon2id scheme=3 estructural: H' variable-length Blake2b, G compression (BlaMka), main loop con indexing híbrido data-dependent/independent (Argon2id). Sin matchear vector RFC §A.3 todavía (ADR-0060). Migrar via export/import. |
| `29` | 🔴 Rechazado | Bloque Z1d (2026-05-29). Blake2b RFC 7693 puro-Rust como cimiento para Z1e+ (H' variable-length necesario para Argon2id). Sin habilitar Argon2id todavía — solo el primitive ready. (ADR-0058). Migrar via export/import. |
| `28` | 🔴 Rechazado | Bloque Z1c (2026-05-29). Implementación scrypt RFC 7914 puro-Rust (Salsa20/8 core + BlockMix + ROMix). PASSWORD_SCHEME_SCRYPT=2 + verify dispatch sobre scheme byte. Salt 32 bytes + parámetros `N=16384, r=8, p=1` por default (ADR-0056). Migrar via export/import. |
| `27` | 🔴 Rechazado | Bloque Z3b (2026-05-29). `PolicyMeta` agrega `with_check_sql: Option<String>` + acepta `FOR INSERT` (que antes era rechazado por solo aceptar USING). Habilita `CREATE POLICY ... WITH CHECK (expr)` para enforcement en write-paths. `[GBY-4138]` POLICY_CHECK_VIOLATION (ADR-0054). Migrar via export/import. |
| `26` | 🔴 Rechazado | Bloque Z1b (2026-05-29). Reescritura completa de `UserMeta` on-disk: byte de scheme (1=PBKDF2-SHA256 default + 2/3 placeholders pre-Z1c/e) + `salt: Vec<u8>` + `hash: Vec<u8>` + `iterations: u32`. PBKDF2-SHA256 puro-Rust con 100K iteraciones default. `[GBY-4137]` AUTH_PASSWORD_INCORRECT (ADR-0053). Migrar via export/import. |
| `25` | 🔴 Rechazado | Bloque Z3 (2026-05-29). Row-Level Security. `ObjectKind::Policy` con `PolicyMeta { name, table, action: SELECT\|INSERT\|UPDATE\|DELETE\|ALL, roles: Vec<String>, using_sql: String }`. Hook enforcement en `exec_select/update/delete`. `[GBY-4133..4136]` (ADR-0052). Migrar via export/import. |
| `24` | 🔴 Rechazado | Bloque Z2 (2026-05-29). `ObjectKind::Grant` con `GrantMeta { grantee, object, privs: u32 (bitmask) }`. Privs: `SELECT/INSERT/UPDATE/DELETE/TRUNCATE`. `GRANT`/`REVOKE` + `SET SESSION AUTHORIZATION 'name'`. Enforcement hooks en exec_select/insert/update/delete/truncate. `[GBY-4129..4132]` (ADR-0051). Migrar via export/import. |
| `23` | 🔴 Rechazado | Bloque Z1 (2026-05-29). `ObjectKind::User` + `ObjectKind::Role` (códigos 5/6 en el catálogo). `UserMeta { name, password_hash: u64, salt: u64 }` con FNV-1a-64 sobre `(salt \|\| password)` — **bookkeeping SQL-level, NO crypto-grade** (Z1b lo upgrade a PBKDF2). `[GBY-4124..4128]` (ADR-0050). Migrar via export/import. |
| `22` | 🔴 Rechazado | Bloque Y6 (2026-05-29). Nuevo `ColumnType::Decimal` (code=11) con encoding propio (i128 LE = 16 bytes + scale u8 = 1 byte por fila). `Value::Decimal { value, scale }`. Por-columna `(precision, scale)` con flag `COLUMN_FLAG_HAS_DECIMAL_META = 0x20` + 2 bytes. `DECIMAL`/`NUMERIC`/`DEC` ya no son aliases de FLOAT. `[GBY-4123]` si la parte entera excede precisión (ADR-0046). Rechaza VERSION 21 con `[GBY-1003]`. |
